...
@shared_task(bind=True, name='task_extract_files', priority=5)
...
@shared_task(bind=True, name='task_move_file', priority=0)
...
def task_init(self, filename):
    ...
    if not filename.endswith('.sha256'):
        ...
        if original_filename != sanitize_filename:
            task_move_file.delay(filename, f"{env['ARTIFACT_PATH']}/{sanitize_filename}",)

        # 1st task : generate hash
        task_sha256.delay(f"{env['ARTIFACT_PATH']}/{sanitize_filename}",)

        # 2nd task : Workflow by evidence type
        evidence_type = determine_evidence_type(original_filename)
        if evidence_type == "VELOCIRAPTOR":
            task_extract_files.delay(f"{env['ARTIFACT_PATH']}/{sanitize_filename}", f"{env['SCAN_OUTPUT_PATH']}/{sanitize_filename}_extracted", env['VELOCIRAPTOR_EVIDENCE_PASSWORD'])
